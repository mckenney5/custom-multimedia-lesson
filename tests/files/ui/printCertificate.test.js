...
test.describe("printCertificate", () => {
	...
	test("should return early when print area not found", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = { studentName: "Student", certConfig: {}, overallGrade: { ratio: 0.5 } };
	...
	test("should escape HTML in certConfig.body to prevent injection", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				studentName: "John",
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { body: "<em>Congratulations</em> {{studentName}}" },
				minimumMinutes: 0,
			};
			ui.printCertificate();
	...
	test("should replace {{studentName}} placeholder", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "John Doe", overallGrade: { ratio: 0.85 }, totalSeconds: 0, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasStudentName: printArea.innerHTML.includes("John Doe") };
		});
	...
	test("should replace {{score}} placeholder", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student", overallGrade: { ratio: 0.92 }, totalSeconds: 0, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasScore: printArea.innerHTML.includes("92%") };
		});
	...
	test("should replace {{date}} placeholder", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student", overallGrade: { ratio: 0.5 }, totalSeconds: 0, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			const today = new Date().toLocaleDateString();
			return { hasDate: printArea.innerHTML.includes(today) };
		});
	...
	test("should replace {{totalMinutes}} placeholder", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student", overallGrade: { ratio: 0.5 }, totalSeconds: 150, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasMinutes: printArea.innerHTML.includes("2") };
		});
	...
	test("should replace {{totalHours}} placeholder", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student", overallGrade: { ratio: 0.5 }, totalSeconds: 7200, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasHours: printArea.innerHTML.includes("2") };
		});
	...
	test("should replace {{minimumLength}} placeholder", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student", overallGrade: { ratio: 0.5 }, totalSeconds: 0, certConfig: {}, minimumMinutes: 30,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasMinimumLength: printArea.innerHTML.includes("30") };
		});
	...
	test("should include logo when logoUrl is set", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { logoUrl: "https://example.com/logo.png" },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should not include logo when logoUrl is not set", async () => {
		const result = await page.evaluate(() => {
			...
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasImgTag: printArea.innerHTML.includes("<img") };
		});
	...
	test("should include signature when signatureUrl is set", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { signatureUrl: "https://example.com/sig.png" },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should not include signature when signatureUrl is not set", async () => {
		const result = await page.evaluate(() => {
			...
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasSigText: printArea.innerHTML.includes("Instructor Signature") };
		});
	...
	test("should include watermark when watermarkUrl is set", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { watermarkUrl: "https://example.com/wm.png" },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should not include watermark when watermarkUrl is not set", async () => {
		const result = await page.evaluate(() => {
			...
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { imgCount: (printArea.innerHTML.match(/<img/g) || []).length };
		});
	...
	test("should use custom title from certConfig", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { title: "My Custom Certificate" },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should call window.print()", async () => {
		const result = await page.evaluate(() => {
			...
			return { printCalled: printCalled.length > 0 };
		});
	...
	test("should use default student name when studentName is empty", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "", overallGrade: { ratio: 0.5 }, totalSeconds: 0, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { hasDefaultName: printArea.innerHTML.includes("Student") };
		});
	...
	test("should handle zero totalCourseSeconds", async () => {
		const result = await page.evaluate(() => {
			...
			const html = printArea.innerHTML;
			return {
				hasZeroMinutes: html.includes("Date"),
				hasZeroHours: true,
			};
		});
	...
	test("should calculate score from overallGrade", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student", overallGrade: { ratio: 0.752 }, totalSeconds: 0, certConfig: {}, minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
			return { scoreShown: printArea.innerHTML.includes("75%") };
		});

		expect(result.scoreShown).toBe(true);
	});

	test("should display 0% when overallGrade returns NaN ratio", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			ui._printData = {
				studentName: "Student",
				overallGrade: { ratio: NaN, earnedScore: 0, maxScore: 0 },
				totalSeconds: 0,
				certConfig: {},
				minimumMinutes: 0,
			};
	...
	test("should escape double-quotes in logoUrl to prevent attribute breakout", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { logoUrl: 'https://example.com/" onerror="alert(1)"' },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should escape double-quotes in signatureUrl to prevent attribute breakout", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { signatureUrl: 'https://example.com/" onerror="alert(1)"' },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should escape double-quotes in watermarkUrl to prevent attribute breakout", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { watermarkUrl: 'https://example.com/" onerror="alert(1)"' },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should escape certConfig.title to prevent HTML injection", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { title: "<script>alert(1)</script>" },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should escape body content to prevent HTML injection", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.5 },
				totalSeconds: 0,
				certConfig: { body: '<img src=x onerror="alert(1)"> {{studentName}}' },
				minimumMinutes: 0,
			};
			ui.printCertificate();
			const printArea = document.getElementById("certificate-print-area");
	...
	test("should handle concurrent injection of all placeholders with malicious payloads", async () => {
		const result = await page.evaluate(() => {
			...
			ui._printData = {
				...
				overallGrade: { ratio: 0.75 },
				totalSeconds: 3600,
				certConfig: { body: "{{studentName}} {{score}} {{date}} {{totalMinutes}} {{totalHours}} {{minimumLength}}" },
				minimumMinutes: 30,
			};
			ui.printCertificate();
			Date.prototype.toLocaleDateString = realDate;
	...
	test("should return early when _printData is null", async () => {
		const result = await page.evaluate(() => {
			if(!ui.infoBanner) ui.init();
			const errors = [];
			const originalError = console.error;
			...
			console.error = originalError;
			return { errorLogged: errors.length > 0 && errors[0].includes("_printData not set") };
		});
