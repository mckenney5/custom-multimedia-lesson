...
test.describe("Complete course with failing grade", () => {
	test.setTimeout(90000);

	test("should complete all pages and show failed end screen", async ({ page }) => {
		...
		await test.step("multi_example page: answer quizzes with mostly wrong answers", async () => {
			...
			await quiz1.locator("#Q2_text").fill("5");
			await quiz1.locator(".btn-submit").click();

			await quiz2.locator("label").filter({ hasText: "blue" }).click();
			await quiz2.locator("label").filter({ hasText: "0.15mg" }).click();
		...
		await test.step("multi_example2 page: play video, answer quizzes with mostly wrong answers", async () => {
			...
			await quiz3.locator("label").filter({ hasText: "3" }).click();
			await quiz3.locator(".btn-submit").click();

			await quiz4.locator("label").filter({ hasText: "blue" }).click();
			await quiz4.locator("label").filter({ hasText: "1mg" }).click();
			await quiz4.locator("label").filter({ hasText: "0.4g" }).click();
		...
		// === END SCREEN ===
		await test.step("end screen: verify failed state", async () => {
			...
			await expect(helpOverlay.locator("#help-content")).toContainText("58%");
